...
- `src/mybroker/appliance.py`: build local personal-analyst surfaces including `/today`, memory, journal, role-based task queue, run ledger, cross-day handoff proof, workflow-pattern radar, pattern evidence intake, local dry-run proof, notification dry-run, phone access guidance, and scheduler proofs.
