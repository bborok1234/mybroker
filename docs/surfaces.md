...
Generated by `mybroker brief`, `mybroker appliance home`, `mybroker appliance briefing`, `mybroker appliance briefing-response-apply`, `mybroker appliance briefing-inbox-apply`, `mybroker appliance access-verify`, `mybroker appliance today`, `mybroker appliance agenda`, `mybroker appliance source-refresh`, `mybroker appliance source-refresh-execution`, `mybroker appliance readiness`, `mybroker appliance scheduler summary`, `mybroker appliance run-ledger`, `mybroker appliance handoff`, `mybroker appliance handoff-study-resolution`, `mybroker appliance handoff-response-apply`, `mybroker appliance journal`, `mybroker appliance learning`, `mybroker appliance council`, `mybroker appliance council-response-apply`, `mybroker appliance tasks`, `mybroker appliance task-ledger`, `mybroker appliance review`, `mybroker appliance review-prompt`, `mybroker appliance review-effect`, `mybroker appliance review-response-apply`, `mybroker appliance audit`, `mybroker appliance pattern-radar`, `mybroker appliance pattern-evidence-intake`, and `mybroker appliance pattern-dry-run`, these are the beginner-facing user surfaces. They show market/narrative maps, key themes, three scenario paths, persona viewpoints, beginner explanations, next inspection questions, daily operating order, daily reading order, private phone access readiness, source fan-out, source refresh judgment, final source-execution confirmation, freshness/readiness status, scheduler operations status, run heartbeat/canonical-run proof, cross-day continuity proof, handoff study closure candidates, role-specific analyst notes, beginner learning ledger, role agreement/disagreement, queued analyst work, task status history, local review memory, copy-ready feedback prompts, review-effect proof, copied-feedback handoff proof, briefing reply proof, local briefing inbox proof, handoff response apply proof, council feedback proof, memory audit risks, workflow-pattern adoption queue, workflow-pattern evidence intake, local dry-run proof, copy-ready phone/message handoff, and research-only action candidates.
...
`reports/product/daily-briefing.html` is not a full dashboard and not a notification sender. It is the compressed phone/message handoff packet for the day: headline topic, why now, trust/caveat status, first links, and one copy-ready message for Telegram, iMessage, Slack, or manual paste. It reads existing artifacts only. Real notification send remains a separate approval-gated operation.

`reports/product/briefing-response-apply.html` is not a chat client and not a notification reply handler. It is the local proof after a copied briefing response records review memory, refreshes scout/effect proof, regenerates daily-home, and regenerates daily-briefing in the same run. It does not send messages, call live sources, write host scheduler state, access credentials, or touch accounts.

`reports/product/briefing-inbox-apply.html` is not a messaging integration, chat client, or inbox poller. It is the local proof after MyBroker reads a plain local briefing inbox file, selects the latest valid reply, applies it through the same local briefing response path, and shows which lines were accepted or ignored. It does not fetch messages, send messages, call live sources, write host scheduler state, access credentials, or touch accounts.
...
`reports/product/source-refresh-execution.html` is not the live data runner. It is the final-confirmation surface after approval/preflight proof exists: approval status, preflight status, blockers, stop conditions, rollback/retry rules, and the separately confirmed live-run command preview. It reads local artifacts only and must not imply that approval response alone authorizes execution.
...
`reports/product/pattern-evidence-intake.html` is not a market brief and not a project tracker. It is the local method-adoption intake for a fast-changing agent ecosystem: each researched case and queued candidate is classified as already verified, local candidate, approval-gated, or blocked. It exists to prevent stale repeated recommendations and to keep new Hermes/OpenClaw/MiroFish/TradingAgents-style ideas from changing the daily loop until local proof exists.
...
- `daily_briefing_packet.v1`
- `operator_briefing_response_apply.v1`
- `operator_briefing_inbox_apply.v1`
- `daily_readiness.v1`
