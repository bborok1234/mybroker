...
Generated by `mybroker brief`, `mybroker appliance today`, `mybroker appliance agenda`, `mybroker appliance readiness`, `mybroker appliance scheduler summary`, `mybroker appliance journal`, `mybroker appliance tasks`, and `mybroker appliance task-ledger`, these are the beginner-facing user surfaces. They show market/narrative maps, key themes, three scenario paths, persona viewpoints, beginner explanations, next inspection questions, daily reading order, source fan-out, freshness/readiness status, scheduler operations status, role-specific analyst notes, queued analyst work, task status history, and research-only action candidates.
