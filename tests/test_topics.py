...
class TopicResearchLoopTests(unittest.TestCase):
    ...
    def test_research_plan_and_collection_update_topic_memory(self) -> None:
        ...
        self.assertEqual(scout["autonomous_start"]["mode"], "system_recommends_first_topic")
        self.assertFalse(scout["autonomous_start"]["operator_input_required"])
        self.assertGreaterEqual(len(scout["pattern_watch"]), 1)
        self.assertIn("operator_brief", scout["recommended_topic"])
        self.assertIn("why_today", scout["recommended_topic"]["operator_brief"])
        self.assertGreaterEqual(len(scout["recommended_topic"]["beginner_reading_order"]), 3)
        self.assertTrue(any(response["intent"] == "more" for response in scout["recommended_topic"]["copy_ready_responses"]))
