...
3. MyBroker product surfaces: beginner-facing MiroFish-inspired market brief and local analyst screens generated by `mybroker brief`, `mybroker appliance home`, `mybroker appliance today`, `mybroker appliance agenda`, `mybroker appliance source-refresh`, `mybroker appliance source-refresh-execution`, `mybroker appliance readiness`, `mybroker appliance scheduler summary`, `mybroker appliance run-ledger`, `mybroker appliance handoff`, `mybroker appliance handoff-study-resolution`, `mybroker appliance handoff-response-apply`, `mybroker appliance journal`, `mybroker appliance tasks`, `mybroker appliance task-ledger`, `mybroker appliance review`, `mybroker appliance review-prompt`, `mybroker appliance review-effect`, `mybroker appliance pattern-radar`, `mybroker appliance pattern-evidence-intake`, and `mybroker appliance pattern-dry-run`.
...
- `reports/runtime/pattern-evidence-intake.json` and `reports/product/pattern-evidence-intake.html`: local method-adoption intake that classifies researched agent/investment workflow patterns as already verified, local candidate, approval-gated, or blocked before the scout repeats or promotes them;
- `reports/runtime/pattern-dry-run-proof.json` and `reports/product/pattern-dry-run.html`: local proof that queued workflow-pattern candidates have validated artifacts and phone surfaces before deeper adoption;
...
- `reports/runtime/source-refresh-execution-brief.json` and `reports/product/source-refresh-execution.html`: final-confirmation packet between approval/preflight and actual live source execution. It reads local proof artifacts only, shows stop conditions, rollback, blockers, and the separately confirmed live-run command preview;
- `appliance source-refresh-response`: a one-command local handoff from the phone's copied approval response into refreshed live-run proof, preflight proof, source refresh briefing, and source refresh execution brief. It does not execute live network calls;
...
`source-refresh.html` plus `source-refresh-execution.html` in one local step. Even with
`--intend-execute --confirm-live-network`, this handoff records readiness only; it does not fetch
live data. `source-refresh-execution.html` is the final local confirmation screen: only when it says
`ready_for_final_confirmation` should a separately confirmed `source-refresh-live-run --execute
--confirm-live-network` path be considered.
